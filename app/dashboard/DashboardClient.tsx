                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {Object.values(THEMES).map((theme) => {
                      ...
                      return (
                        ...
                        >
                          {/* Theme Background, Pattern, and Glow Preview */}
                          <div className={cn("absolute inset-0 z-0 select-none pointer-events-none transition-transform duration-500 group-hover:scale-105", theme.bgClass.replace("min-h-screen", "").replace("relative", "").replace("overflow-hidden", ""))} />
                          <div className={cn("absolute inset-0 z-0 opacity-40 select-none pointer-events-none", theme.patternClass)} />
                          <div className={cn("absolute inset-0 z-0 select-none pointer-events-none blur-xl opacity-60 scale-75", theme.glowClass)} />
                          ...
                          <div className="relative z-10 flex flex-col justify-between h-full w-full p-5">
                            ...
                            <div className="flex items-start justify-between w-full">
                              <div className="flex flex-col gap-1">
                                <span className={cn("font-bold text-base tracking-wide transition-all group-hover:translate-x-0.5", theme.textPrimary, theme.fontClass)}>
                                  {theme.name}
                                </span>
                                <span className={cn("text-[9px] uppercase font-mono tracking-wider opacity-60 bg-current bg-clip-text font-bold", theme.textSecondary)}>
                                  Glassmorphic
                                </span>
                              </div>
                              {isActive ? (
                                ...
                                <span className="size-6 rounded-full border border-white/20 bg-black/25 opacity-0 group-hover:opacity-100 transition-all duration-200 shrink-0 flex items-center justify-center">
                                  <span className="text-[9px] font-medium text-white/90">Pick</span>
                                </span>
                              )}
                            </div>

                            {/* Bottom row: Emoji representation */}
